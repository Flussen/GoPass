...
const Groups: React.FC<GroupProps> = ({ showDashboard, setShowProfile, setShowDashboard, setOptionName, optionName, userKey, userName, setTheme, theme }) => {
        ...
        <div className=" flex w-full space-x-12 h-full ">
          <div id="MyGroups" className="flex flex-col justify-start  w-full font-semibold space-y-5 text mb-5">
            <div className="flex justify-between  items-center">
              <div className="text-responsivo dark:text-whitebg text-black font-bold">
                My <span className="  text-primary">Groups</span>
              </div>
              <div className="flex items-center space-x-6">
                <div className="flex justify-center items-center text-base space-x-2 dark:text-gray dark:hover:text-whitebg dark:bg-darkgray text-whitegray hover:text-gray bg-white  h-12 rounded-full w-24 cursor-pointer">
                  <div>
                    Sort
                  </div>
                  <SortRoundedIcon sx={{ fontSize: 24 }} />
                </div>
                <div onClick={()=>setTwoColum(false)}  className={`flex justify-center items-center text-base space-x-2 ${!twoColum? 'dark:text-black text-whitebg   bg-primary':'dark:text-gray dark:hover:text-whitebg dark:bg-darkgray text-whitegray hover:text-gray bg-white '}  h-12  rounded-full w-12 cursor-pointer`}>
